#include <stdlib.h>

#include <pocas/core/list.h>
#include <pocas/core/string.h>

#include "testresult_internal.h"

struct TestResult
{
    TestResultCode code;
    char *message;
    List *ignoredMessages;
};

TestResult *TestResult_create(void)
{
    TestResult *self = malloc(sizeof(TestResult));
    self->code = TRC_NONE;
    self->message = 0;
    self->ignoredMessages = 0;
    return self;
}

TestResultCode TestResult_code(const TestResult *self)
{
    return self->code;
}

void TestResult_setCode(TestResult *self, TestResultCode code)
{
    self->code = code;
}

const char *TestResult_message(const TestResult *self)
{
    return self->message;
}

void TestResult_setMessage(TestResult *self, const char *message)
{
    free(self->message);
    self->message = String_copy(message);
}


ListIterator *TestResult_ignoredMessages(const TestResult *self)
{
    if (!self->ignoredMessages) return 0;
    return List_iterator(self->ignoredMessages);
}

void TestResult_addIgnoredMessage(TestResult *self, const char *message)
{
    if (!self->ignoredMessages) self->ignoredMessages = List_createStr(0);
    List_append(self->ignoredMessages, String_copy(message));
}

void TestResult_destroy(TestResult *self)
{
    if (!self) return;
    List_destroy(self->ignoredMessages);
    free(self->message);
    free(self);
}
